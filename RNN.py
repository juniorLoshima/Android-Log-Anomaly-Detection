...
# Transformer les données en séquences pour le RNN
...
model = Sequential()
model.add(LSTM(64, return_sequences=True, input_shape=(X_sequences.shape[1], X_sequences.shape[2])))
...
anomaly_timestamps = logs_df.iloc[anomalies + sequence_length - 1]['timestamp']  # Ajuster l'index pour correspondre aux séquences
...
plt.title("Detection des anomalies dans les logs Android avec un RNN (LSTM)")
...
anomaly_df.to_csv('rnn_anomalies_report.csv', index=False)
